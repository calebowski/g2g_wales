...
## read in mod and obs
sim_ts <- read.time.series(sim_files[13:22],sim_path, output = "model")
sufi_ts <- read.time.series(sufi_files[13:22],sufi_path, output = "model")
obs_ts <- read.time.series(sufi_files[13:22],sufi_path, output = "observed")
...
for (g2g_id in poor_sites){
  cat("Starting plot for g2g site:", g2g_id, "\n")
    ...
    ggsave(
      filename = file.path(path, paste0(g2g_id, "__2012_2021_hydrograph.png")),
      plot = combined,
      width = 12,
      height = 18,
      units = "in",
...
stats_dt[model == "sufi", .(G2G.ID, r)]

p <- plot.single.point(poor_sites, cdata = wales_cdata, wales, fill.variable = )
